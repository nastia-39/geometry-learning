...
\begin{abstract}
	...
	%
	In this paper, we show how a Transformer-like Energy-Based Model (TEBM) can provably learn a Gibbs distribution over a function space $p_\theta(f) \propto \exp(-\energy_{\theta}(f))$ from noisy observations $f_j(x_i) + \epsilon$.
	%
	We consider a few examples of linear families of energy functionals and show how in the mean-field regime, where the number of heads $\to \infty$, training a TEBM recovers the parameter $\theta$ with provable approximation and generalization guarantees.
	%
...
\section{Linear Energy Functionals}\label{sec:linear-energy-functionals}
...
\begin{assumption}[Linear energy functional]
	We assume that the energy functional $E_{\theta}$ is linear in $\theta$:
	\[
		E_{\theta}(f) = \langle \theta, \Phi(f) \rangle
	\]
	where $\Phi$ is a statistic extractor on $\ffam$.
\end{assumption}


Note, that in the general case, learning the energy functional $E_{\theta}$ is a hard problem because the partition function $Z_{\theta}$ is intractable. However, in the case of linear energy functionals for families of functions that we consider in this paper (see \Cref{sec:families-of-energy-functionals}), this term is a lot easier to compute and analyze.

\begin{lemma}
	\begin{align*}
		\nabla_\theta \log Z(\theta)=-\E_{f \sim \nu_\theta}[\Phi(f)]
	\end{align*}
\end{lemma}
...
\subsection{Families Of Energy Functionals}\label{sec:families-of-energy-functionals}
...
% Let $\mathcal{Q} \subset \mathbf{S}_+^d$ be \todo{define}. Given a PSD matrix $Q$, let $\Delta_Q$ denotes the anisotropic Laplacian operator: $\Delta_Q f := \nabla\cdot(Q\nabla f)$. Total variation norm for a function (\todo{or a Radon measure}) $g:\domain \to \Real$ is defined as $\|g\|_{\mathcal M} := \int\limits_{x \in \domain} |g(x)| \, dx$.
...
Let $\theta$ be a measure over $\mathcal{Q}$. Let statistic $\Phi : H^1(M) \to \mathbb{R}^k$ be defined as $\Phi(f)(Q) := \int_{x \in \domain} \| \nabla f(x) \|^2_{Q} \, dx$. Then:
\begin{gather*}
...
\begin{table}[h]
	\centering
	\begin{tabular}{p{2cm}p{3.6cm}p{3.6cm}p{4.6cm}}
		\toprule
		Energy type                &
...
\begin{align*}
	\max_{\theta \in \Theta} p_\theta(\dataset) = \min_{\theta \in \Theta} \frac{1}{\nfunctions} \sum_{i=1}^{\nfunctions} \energy_{\theta}(f_i) + \log Z(\theta)
\end{align*}
...
Now, back to the original problem, where each function $f$ is given by its values i $n$ poitns $x_i$: $y_i = f(x_i) + \epsilon_i$, $\epsilon_i \overset{\text{i.i.d.}}{\sim} \mathcal{N}(0, \sigma^2)$.
% Note, that
% \begin{align*}
...
\begin{gather}\label{eq:loss-w-data}
	\max_{\theta \in \Theta} p_\theta(\dataset) =
	\min_{\theta \in \Theta}
	-\frac{1}{\nfunctions} \sum_{i=1}^{\nfunctions}
	% \underbrace{
...
Unfortunately, in practice each term $\log p_\theta\left(D^{(i)} \right)$ is intractable as
\begin{align*}
	\log p_\theta\left(D \right) = \log \int_{f \in \ffam} \exp \left(-\log p\left(D \mid f\right)-\energy_{\theta}(f)\right) \nu(d f)
\end{align*}
...
$
	L_\theta(f, D)
	:=
	\ell(D,f)+ \energy_{\theta}(f),
$
where $\ell(D,f)$ denotes the empirical negative log-likelihood and
$\energy_{\theta}$ is an energy functional.
As the expression $-\log p(D \mid f) - \energy_{\theta}(f)$ is strongly convex, the Laplace approximation of $\log p_\theta\left(D \mid f\right)$ gives us
\begin{equation}
	\label{eq:laplace-result}
	- \log p_\theta\left(D \right)
	\;\approx\;
	\underbrace{L_\theta(\hat f_\theta, D)}_{\text{MAP cost}}
	\;+\;
	\underbrace{\tfrac{1}{2}\log \det H_\theta}_{\text{complexity penalty}}
	% =
...
\end{equation}
where $\hat f_\theta = \arg\min_f L_\theta(f,D)$ is the MAP estimator
and \todo{fix}
$H_\theta = \nabla^2_f \ell(D, \hat f_\theta) + \nabla^2 \energy_{\theta}^{\rm sm}(\hat f_\theta)$.
...
% Note, for a fixed $\theta$ the minimizer $f$ of $L_\theta(f, D)$ is unique:
% $
% 	\hat f_\theta
% 	=
...
% Substituting this approximation \eqref{eq:laplace-result} into the
...
Using this approximation, we can turn the intractable convex optimization problem \eqref{eq:loss-w-data} into a tractable bilevel non-convex surrogate of it.
%
